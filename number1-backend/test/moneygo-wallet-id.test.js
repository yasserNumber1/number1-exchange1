...
test('MoneyGo IDs require uppercase U and a non-space suffix', () => {
  for (const id of ['U1', 'UABC123', ' UABC123 ']) assert.equal(isValidMoneyGoWalletId(id), true);
  for (const id of ['', 'U', 'u123', 'A123', 'UA B', null, 123]) assert.equal(isValidMoneyGoWalletId(id), false);
});
