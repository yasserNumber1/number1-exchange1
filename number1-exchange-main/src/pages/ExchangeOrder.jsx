  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', textAlign:'center', padding:'48px 24px 32px', gap:20 }}>
      ...
      <div style={{ width:100, height:100, borderRadius:'50%', background:'linear-gradient(135deg,rgba(0,229,160,0.15),rgba(0,210,255,0.1))', border:'2.5px solid #00e5a0', display:'flex', alignItems:'center', justifyContent:'center', animation:'eo-pop 0.6s cubic-bezier(0.34,1.56,0.64,1) both, eo-glow 2s ease-in-out 0.6s infinite' }}>
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#00e5a0" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
      </div>
        ...
        <div className="eo-status-card">
          ...
          {isApproved && !isCompleted && (
            <div style={{
              ...
              <div style={{ width:32, height:32, borderRadius:'50%', background:'rgba(245,158,11,0.15)', border:'2px solid #f59e0b', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
        ...
        <div className="eo-card">
          ...
          <div className="eo-status-steps">
            {[
              ...
              const isActive_    = step.key === currentStatus && !isDone
              const isCompleted_ = stepIdx < curIdx || (currentStatus === 'completed' && stepIdx <= order_.indexOf('completed'))
              return (
                <div key={step.key} className={`eo-status-step ${isActive_ ? 'eo-status-step--active' : ''} ${isCompleted_ ? 'eo-status-step--done' : ''}`}>
                  <div className="eo-status-step-dot">
                    {isCompleted_ ? <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
                      : isActive_ ? <span className="eo-pulse-dot" />
...
const CSS = `
  ...
  .eo-header { display: flex; align-items: center; justify-content: space-between; padding: 14px 20px; background: var(--card); border-bottom: 1px solid var(--border-1); position: sticky; top: 0; z-index: 40; }
  .eo-back { display: flex; align-items: center; gap: 6px; padding: 7px 14px; border-radius: 9px; border: 1px solid var(--border-1); background: transparent; color: var(--text-2); cursor: pointer; font-size: 0.85rem; font-weight: 600; font-family: 'Cairo',sans-serif; transition: all 0.15s; }
  ...
  .eo-steps { display: flex; align-items: center; justify-content: center; padding: 12px 24px; background: var(--card); border-bottom: 1px solid var(--border-1); }
  ...
  .eo-dot { width: 26px; height: 26px; border-radius: 50%; flex-shrink: 0; background: var(--cyan-dim); border: 1.5px solid var(--cyan); display: flex; align-items: center; justify-content: center; font-size: 0.72rem; font-weight: 700; color: var(--cyan); }
  ...
  .eo-status-step { display: flex; align-items: flex-start; gap: 12px; }
