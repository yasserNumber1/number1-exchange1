...
function WithdrawModal({ isOpen, onClose, balance }) {
  ...
  const [infoLoading,  setInfoLoading]  = useState(true)
  const whatsappNumber = normalizeWhatsappNumber(withdrawInfo?.whatsapp)
  const whatsappAvailable = withdrawInfo?.whatsappEnabled !== false && Boolean(whatsappNumber)
  const whatsappUnavailableText = lang === 'ar'
    ? (withdrawInfo?.whatsappUnavailableMessageAr || 'متاح قريبًا')
    : (withdrawInfo?.whatsappUnavailableMessageEn || 'Coming soon')
        ...
        <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {infoLoading ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <button onClick={openWhatsApp} disabled={!amountValid || !whatsappAvailable}
                  style={{ padding: '13px 16px', borderRadius: 12, cursor: amountValid && whatsappAvailable ? 'pointer' : 'not-allowed', fontFamily: "'Tajawal',sans-serif", fontSize: '0.9rem', fontWeight: 700, background: amountValid && whatsappAvailable ? 'rgba(37,211,102,0.1)' : 'rgba(255,255,255,0.03)', border: `1px solid ${amountValid && whatsappAvailable ? 'rgba(37,211,102,0.3)' : 'var(--border-1)'}`, color: amountValid && whatsappAvailable ? '#25d366' : 'var(--text-3)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: amountValid ? 1 : 0.5 }}>
                  ...
                  {whatsappAvailable ? (lang === 'ar' ? 'واتساب' : 'WhatsApp') : whatsappUnavailableText}
                </button>
