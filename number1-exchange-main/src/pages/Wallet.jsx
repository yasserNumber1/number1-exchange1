// ============================================
// src/pages/Wallet.jsx — محفظة المستخدم
// الإصدار: 2.0 — يدمج USDT القديم + N1 Credit الجديد
// ============================================
  ...
  return (
    ...
    >
      <div style={{
        ...
        {/* Header */}
        <div style={{ padding: '22px 24px 16px', borderBottom: '1px solid var(--border-1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ padding: 22, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* نوع الإيداع */}
          <div>
            <label style={{ display: 'block', fontSize: '0.72rem', color: 'var(--text-3)', marginBottom: 8, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1 }}>
              نوع الإيداع
            </label>
          ...
          {type === 'usdt' && (
            <div>
              ...
              <input
                ...
                onBlur={e  => e.target.style.borderColor = 'var(--border-1)'}
              />
              <div style={{ marginTop: 5, fontSize: '0.68rem', color: 'var(--text-3)', fontFamily: "'JetBrains Mono',monospace" }}>
                TRC20 Network Only
              </div>
            </div>
          )}
          ...
          {type === 'bank_transfer' && (
            <div>
              ...
              <div
                ...
                style={{ border: `1.5px dashed ${receipt ? 'var(--green)' : 'var(--border-2)'}`, borderRadius: 12, padding: receipt ? 10 : 24, textAlign: 'center', cursor: 'pointer', background: receipt ? 'rgba(0,229,160,0.04)' : 'rgba(0,0,0,0.1)', transition: 'all 0.2s' }}
              >
                {preview ? (
                  <div>
                    <img src={preview} alt="receipt" style={{ width: '100%', maxHeight: 130, objectFit: 'contain', borderRadius: 8 }} />
                    <div style={{ marginTop: 6, fontSize: '0.72rem', color: 'var(--green)', fontFamily: "'JetBrains Mono',monospace" }}>✓ {receipt.name}</div>
                  </div>
                ) : (
                  <>
                    <div style={{ fontSize: '1.8rem', marginBottom: 6 }}>📎</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-2)', fontFamily: "'Tajawal',sans-serif" }}>اضغط لرفع الإيصال</div>
          ...
          {/* خطأ */}
          {error && (
            <div style={{ padding: '10px 14px', background: 'rgba(248,81,73,0.08)', border: '1px solid rgba(248,81,73,0.2)', borderRadius: 10, fontSize: '0.82rem', color: '#f85149', display: 'flex', alignItems: 'center', gap: 8, fontFamily: "'Tajawal',sans-serif" }}>
              ⚠️ {error}
            </div>
          )}
...
export default function WalletPage() {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        setShowWithdraw(false); setWithdrawSuccess(false)
    ...
    <div style={{ minHeight: '80vh', padding: '50px 24px 80px', maxWidth: 720, margin: '0 auto', direction: 'rtl' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#0d2137 0%,#0a1628 100%)', border: '1px solid rgba(0,212,255,0.2)', borderRadius: 20, padding: '28px 24px', marginBottom: 16, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 20 }}>
          <span style={{ fontSize: '2.8rem', fontWeight: 900, color: '#26a17b', fontFamily: "'Orbitron',sans-serif", lineHeight: 1 }}>
        ...
        {!wallet?.isActive && (
          <div style={{ marginTop: 10, textAlign: 'center', fontSize: '0.78rem', color: '#f85149', fontFamily: "'Tajawal',sans-serif" }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,rgba(0,210,255,0.06),rgba(124,92,252,0.06))', border: '1px solid rgba(0,210,255,0.18)', borderRadius: 20, padding: '24px', marginBottom: 24, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 20 }}>
          <span style={{ fontSize: '2.8rem', fontWeight: 900, color: 'var(--cyan)', fontFamily: "'Orbitron',sans-serif", lineHeight: 1, textShadow: '0 0 20px rgba(0,210,255,0.35)' }}>
            {wallet?.n1Balance?.toFixed(2) || '0.00'}
          </span>
      ...
      <div style={{ background: 'var(--card)', border: '1px solid var(--border-1)', borderRadius: 18, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border-1)' }}>
          {[
            { id: 'deposits',     label: 'طلبات الإيداع N1' },
            { id: 'transactions', label: 'معاملات USDT'       },
          ].map(tab => (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {deposits.map((d, i) => (
                    ...
                    >
                      <td style={{ padding: '12px 14px', fontSize: '0.85rem', fontFamily: "'Tajawal',sans-serif" }}>
                        {d.type === 'bank_transfer' ? '🏦 بنكي' : '₮ USDT'}
                      </td>
                      <td style={{ padding: '12px 14px', fontFamily: "'JetBrains Mono',monospace", fontSize: '0.9rem', fontWeight: 700, color: 'var(--cyan)' }}>
                        {Number(d.amount).toFixed(2)}
                      </td>
                      <td style={{ padding: '12px 14px', fontFamily: "'JetBrains Mono',monospace", fontSize: '0.75rem', color: 'var(--text-3)' }}>
                        {d.currency}
                      </td>
              ...
              return (
                <div key={tx._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 20px', borderBottom: '1px solid var(--border-1)' }}>
                  ...
                  <div style={{ textAlign: 'left' }}>
                    ...
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-3)', fontFamily: "'JetBrains Mono',monospace" }}>
      ...
      {/* ══ مودال السحب USDT (القديم محفوظ) ══════ */}
      {showWithdraw && (
        <div
          style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}
          onClick={() => setShowWithdraw(false)}
        >
          <div
            style={{ background: 'var(--card)', border: '1px solid var(--border-1)', borderRadius: 20, padding: 28, width: '100%', maxWidth: 420 }}
          ...
          >
            ...
            {withdrawSuccess ? (
              ...
              <>
                ...
                {withdrawError && (
                  <div style={{ color: '#f85149', fontSize: '0.82rem', fontFamily: "'Tajawal',sans-serif", marginBottom: 12, textAlign: 'center' }}>
                    {withdrawError}
                  </div>
                )}

                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={() => setShowWithdraw(false)} style={{ flex: 1, padding: 12, background: 'none', border: '1px solid var(--border-1)', borderRadius: 10, color: 'var(--text-2)', fontFamily: "'Tajawal',sans-serif", cursor: 'pointer' }}>
                    إلغاء
                  </button>
                  <button
